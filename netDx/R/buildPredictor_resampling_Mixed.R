...
#' Expects some nets to be of full-matrix type (e.g. gene expression)
#' but others of sparse binary type (e.g. CNV). 
#' Currently, for CNV, only shared co-occurrence is accepted.
#' NOTE: To ensure reproducibility, set the RNG seed
...
for (g in subtypes) {
	...
	for (repNum in 1:numResamples) {
		...
		# limit to patients in current resampling
		pdat_tmp	<- pdat[,which(colnames(pdat)%in% pheno_subtype$ID)]
		p_GR_tmp <- p_GR[which(p_GR$ID %in% pheno_subtype$ID)]
