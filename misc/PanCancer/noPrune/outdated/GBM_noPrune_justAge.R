...
megaDir <- sprintf("%s/noPrune_justAge_%s",outRoot,dt)
...
cat("Excluding sex as input param\n")
dats$clinical <- dats$clinical[c("age"),,drop=FALSE]
...
for (rngNum in 1:50) {
	...
	netList2 <- makePSN_NamedMatrix(alldat_train, 
		...
		netDir,simMetric="custom",customFunc=normDiff,writeProfiles=FALSE,
