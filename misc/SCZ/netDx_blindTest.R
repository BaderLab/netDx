...
geneFile <- sprintf("%s/anno/gencode.v27lift37.annotation.gtf.geneidsymbols.txt",
	rootDir)
...
idx <- which(duplicated(xpr_genes))
cat(sprintf("\t%i gene names duplicated; excluding\n", length(idx)))
xpr_genes <- xpr_genes[-idx]
xpr <- xpr[-idx,]
...
ctrlIdx <- which(pheno$Dx %in% "Control")
caseIdx <- which(pheno$Dx %in% "SCZ")

set.seed(123) # make reproducible
idx1 <- sample(ctrlIdx,length(ctrlIdx)/2,F)
idx2 <- sample(caseIdx,length(caseIdx)/2,F)
idx <- c(idx1,idx2)
