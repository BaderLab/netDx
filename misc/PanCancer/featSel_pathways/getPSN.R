...
#' only use where it's possible to have more than one feature selected
#' net
rm(list=ls())
...
stop("Change outDir")
outDir		<- "/Users/shraddhapai/Documents/Research/BaderLab/2017_PanCancer_Survival/oneNetPer_FeatSel/KIRC"  
cutoff 			<-10  	# include nets with score >= this value
pctPass			<- 1.0	# must cross threshold this % of time.
...
# directory with the tmp/ folder that has patient networks 
# 
selIter <- list(
...
for (aggFun in c("MAX")) {
	...
	cat("-----\n")
	pdf("test.pdf")
	aggNetPruned_file <- netDx::writeWeightedNets(ptFile$YES,
					netInfo=netInfo_combinedF,
					poolDir,keepNets=newNetIDs[,2],outDir,
					filterEdgeWt=corrThresh,limitToTop=25,
					outFileName=sprintf("%s/%s_%s_%s_PSN.txt", 
						outDir,curSet,simMode,aggFun),
					writeAggNet=aggFun,verbose=FALSE)
	dev.off()
	
	# run  second time without filtering for edge weight, for Dijkstra
	pdf("test.pdf")
	aggNetFull_file <- netDx::writeWeightedNets(ptFile$YES,
					netInfo=netInfo_combinedF,
					poolDir,keepNets=newNetIDs[,2],outDir,
					filterEdgeWt=0,limitToTop=1000L,
					outFileName=sprintf("%s/dummy_PSN.txt", 
						outDir,curSet,simMode,aggFun),
					writeAggNet=aggFun,verbose=FALSE)
	dev.off()
	...
	aggNet<- read.delim(aggNetFull_file,sep="\t",h=T,as.is=T)[,1:3]
	...
	aggNet_pruned <- read.delim(aggNetPruned_file,sep="\t",h=T,as.is=T)[,1:3]
