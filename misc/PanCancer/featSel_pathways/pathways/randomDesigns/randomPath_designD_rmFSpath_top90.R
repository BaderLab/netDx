...
megaDir <- sprintf("%s/pathRandom_designD_rmFSgenes_topX_%s",outRoot,dt)
...
cat(sprintf("REMOVE FS-like paths: %i FS genes in %i FS paths\n",
	length(fsGenes),length(fsPath)))
torm <- numeric()
for (k in 1:length(pathwayList)) {
	x <- intersect(fsGenes, pathwayList[[k]])
	if (length(x)>0) torm <- c(torm,k)
}
cat(sprintf("%i pathways with 1+ FS genes being removed\n",length(torm)))
pathwayList <- pathwayList[-torm]
...
tryCatch({
for (topX in c(90,95)) {
	megaDir2 <- sprintf("%s/top%i",megaDir,topX)
for (rngNum in 1:50) {
	cat(sprintf("-------------------------------\n"))
	cat(sprintf("RNG seed = %i\n", rngNum))
	cat(sprintf("-------------------------------\n"))
	outDir <- sprintf("%s/rng%i",megaDir2,rngNum)
	dir.create(outDir)
