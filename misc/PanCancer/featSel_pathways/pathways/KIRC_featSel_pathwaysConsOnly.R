...
# VM4
inDir <- "/home/netdx/BaderLab/PanCancer_KIRC/input"
outRoot <- "/home/netdx/BaderLab/PanCancer_KIRC/output"
rootDir <- "/home/netdx/BaderLab"

dt <- format(Sys.Date(),"%y%m%d")
megaDir <- sprintf("%s/featSel_pathways_%s",outRoot,dt)

# files that contain consensus nets
...
for (nm in names(netScoreFile)) {
    ...
    consNets[[nm]] <- netNames[idx]
    cat(sprintf("Consensus nets for %s are:\n",nm))
    print(consNets[[nm]])
    cat("---\n")
}
...
consNets <- unique(unlist(consNets))
consNets <- sub(".profile","",consNets)
pathwayList <- pathwayList[consNets]
cat(sprintf("\tFILTER: %i consensus pathways left\n", length(pathwayList)))
