...
writeQueryBatchFile <- function(netDir, netList, outDir = tempdir(), idFile, orgName = "predictor", 
    ...
    tmp2 <- c("organism", basename(idFile), orgName, orgDesc, orgAlias, as.character(taxID))
    ...
    # write networks
    net_DF <- data.frame(type = "network", filename = netList, name = sub(".txt", 
        "", netList), description = netList, groupCode = groupCode)
    write.table(net_DF, file = outF, sep = "\t", col.names = FALSE, row.names = FALSE, 
