...
megaDir <- sprintf("%s/PCA1netUniv_%s",outRoot,dt)
...
netSets$rna <- rownames(dats$rna)
# apply pruning to RNA data
pdf(sprintf("%s/RNA_prune.pdf",megaDir))
res <- runLM(dats$rna,pheno_all$STATUS)
dev.off()
browser()
res <- subset(res,adj.P.Val < 0.5)
cat(sprintf("\t*** LM pruning, %i variables left\n",
	nrow(res)))
dats$rna <- dats$rna[which(rownames(dats$rna) %in% rownames(res)),]
netSets$rna <- rownames(dats$rna)
