...
corrFeatWithOutcome <- function(pheno, datList, groupList,inputNets,
	selFeatures,numPCs=3,filePfx="corrFeat") {
	...
	for (k in 1:ncol(combs)) {
		...
		p <- ggplot(tmp,aes_string(x=sprintf("PC%i",i),
			y=sprintf("PC%i",j)))
		p <- p + geom_point(aes(colour=factor(STATUS)),alpha=0.6,
...
pdf(sprintf("%s_PCview.pdf", filePfx),height=11,width=11)
tryCatch({
...
pdf(sprintf("%s_PCtable.pdf",filePfx),height=11,width=11)
tryCatch({
