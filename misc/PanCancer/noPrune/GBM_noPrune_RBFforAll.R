...
megaDir <- sprintf("%s/np_RBFall_%s",outRoot,dt)
...
browser()
	netList <- makePSN_NamedMatrix(alldat_train, 
		...
		netDir,simMetric="custom",customFunc=sim.kern,sparsify=TRUE,
	...
	netList2 <- makePSN_NamedMatrix(alldat_train, 
		...
		netDir,simMetric="custom",customFunc=normDiff2,sparsify=TRUE,
		writeProfiles=FALSE,
...
netList <- makePSN_NamedMatrix(alldat,
	...
	simMetric="custom",customFunc=sim.kern,sparsify=TRUE,
	verbose=FALSE,numCores=numCores,writeProfiles=FALSE)
