...
This workflow is identical to that of using unsmoothed somatic mutations (see vignette 3: XXXXX); the only difference is an additional step to smooth binary mutations.
...
We apply minor modification to finish the matrix with sample names and clinical information:
```{r,eval=TRUE}
...
out <- suppressMessages(
	buildPredictor(dataList=dataList,groupList=groupList,
	  ...
	  numCores=20L, featScoreMax=10L, featSelCutoff=9L,
	  numSplits=10L,logging="none"
))
```
