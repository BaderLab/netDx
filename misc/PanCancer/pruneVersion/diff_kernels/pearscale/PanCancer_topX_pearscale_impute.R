	...
	for (nm in names(dats_train)) { 
		...
		if (nrow(dats_train[[nm]])<2)  # clinical only has one var, take it.
			...
			fit <- cv.glmnet(x=t(dats_train[[nm]]),
				y=factor(pheno$STATUS), family="binomial", alpha=0) # ridge
			wt <- abs(coef(fit,s="lambda.min")[,1])
			wt <- wt[order(wt,decreasing=TRUE)]
			wt <- wt[-which(names(wt) %in% "(Intercept)")]
			if (nm %in% "clinical") maxind <- min(topClin,length(wt)-1)
			else maxind <- min(topX,length(wt)-1)
			vars <- names(wt[1:maxind])
		}
