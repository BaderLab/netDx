...
megaDir <- sprintf("%s/PCAmultinet_%s",outRoot,dt)
...
# apply pruning to RNA data
#pdf(sprintf("%s/RNA_prune.pdf",megaDir))
#res <- runLM(dats$rna,pheno_all$STATUS,topVar=50)
#dev.off()
#res <- subset(res,adj.P.Val < 0.5)
#cat(sprintf("\t*** LM pruning, %i variables left\n",
#	nrow(res)))

# now perform PCA
pr <- prcomp(t(na.omit(dats$rna)))
imp <- summary(pr)$importance[3,] # cumulative var
idx <- which.min(abs(imp-0.95))
cat(sprintf("First %i PCs explain 95%% variance, picking projections\n",
	idx))
dats$rna <- t(pr$x[,1:idx])
netSets$rna <- list()
for (k in 1:nrow(dats$rna)) { 
	nm <- rownames(dats$rna)[k]
	netSets$rna[[nm]] <- nm
}
combList$rna <- sprintf("%s_cont",rownames(dats$rna))
combList$clinicalArna <- c("clinical_cont", combList$rna)

### THIS IS IMPORTANT. Unless this is updated, 
### the rna matrix with new measure names will be ignored.
alldat <- do.call("rbind",dats)
...
nonclin <- setdiff(names(netSets),c("clinical","rna"))
...
for (rngNum in 1:10) {
	...
	netList3 <- makePSN_NamedMatrix(alldat_train, 
		rownames(alldat_train),netSets[["rna"]],
		netDir,simMetric="custom",customFunc=normDiff,
		verbose=FALSE,numCores=numCores,
		sparsify=TRUE,append=TRUE)
	netList <- c(netList,netList2,netList3)
