---
title: 'netDx use case: 4-way classification: Medulloblastoma tumour subtype new GM'
output:
  html_document:
    df_print: paged
---
...
numCores    <- 10L       # number of cores for parallel processing
...
2. `GM_createDB()`: All the input networks in `netDir` are integrated into a single GeneMANIA database.
3. `GM_writeQueryFile()`: Write a GeneMANIA query file; the GeneMANIA query will obtain a similarity ranking for test patients
4. `runGeneMANIA()`: Runs a GeneMANIA query
5. `GM_getQueryROC()`: Process the patient ranking output file (the `.prank` file)
...
for (g in names(groupSig)){
    ...
    resFile <- runGeneMANIA3(dbDir$dbDir, qFile, pDir)
