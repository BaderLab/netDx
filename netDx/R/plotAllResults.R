...
featSelNet <- sapply(names(featScores), function(nm) {
	...
	if(nrow(x) != 0){
		...
		y
			...
			cat(sprintf("NOTE: No feature selected nets for %s class, no Emap for this class
						or PSN will be generated\n", nm))
	}
})

feat_sel_chk <- 0
for(cur_name in names(featSelNet)){
	if(is.null(featSelNet[[cur_name]])) feat_sel_chk <- 1
}
