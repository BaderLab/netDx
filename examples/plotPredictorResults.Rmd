...
1. **GMT file:** A file with top-scoring genesets in the GMT format, similar to the example pathway file.
...
Here is the enrichment map showing the top-scoring networks for `SURVIVEYES`:

![Emap SURVIVEYES](./plots/EnrichmentMap_SURVIVEYES.png)

And here is the map for the `SURVIVENO` class:

![Emap SURVIVENO](./plots/EnrichmentMap_SURVIVENO.png){width=50%}

The nodes show top-scoring features, which are here pathways. Edges connect features with shared units; in this context, pathways with shared genes. More intensely coloured nodes are "hotter". 

Outside of this R notebook, the EnrichmentMaps should now be present in the active Cytoscape session. The user may now further customize the network view.
...
```
![View of patient similarity in KIRC using feature-selected networks](./plots/outputPDN.png){width=50%}
