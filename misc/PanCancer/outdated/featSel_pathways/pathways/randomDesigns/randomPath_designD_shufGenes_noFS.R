...
megaDir <- sprintf("%s/pathRandom_designD_shufGenes_noFS_%s",outRoot,dt)
...
fsGenes	<- unique(unlist(fsPath))
uni_genes <- rownames(dats$rna)
uni_genes <- setdiff(uni_genes, fsGenes)
cat(sprintf("%i of %i genes are non-FS, make new universe\n", length(uni_genes),
	nrow(dats$rna)))
...
names(pseudo) <- names(pathwayList)
if (all.equal(lapply(pseudo,length),lapply(pathwayList,length))!=TRUE) {
	cat("real and pseudo pathways have different gene counts\n");
	browser()
}
pathwayList <- pseudo ### replace pathways with pseudo pathways.
