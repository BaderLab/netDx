...
For this example, the predictor will run a **nested cross-validation design**. In nested cross-validation, cross-validation is performed repeatedly over multiple random splits of the data into train and blind test partitions. Feature selected networks are those that consistently score highly across the multiple splits. 
...
# Feature design or Making custom patient similarity networks
...
This section provides more details on these variables.
...
### groupList
This tells the predictor which units should be grouped together in a network. For examples, genes may be grouped into a network representing a pathway. This object is a list; the names match those of `dataList` while each value is itself a list and reflects a potential network.
...
But for clinical data, we want to keep each variable as its own network:

```{r,eval=TRUE}
head(groupList[["clinical"]])
```

To speed up this example, reduce the number of input networks created:
```{r,eval=TRUE}
groupList[["rna"]] <- groupList[["rna"]][1:3]
```

### netDir
This is just the path to the directory where the networks are to be stored.

## The custom similarity function
This function is defined by the user and tells the predictor how to create networks from the provided input data.

Notice that it takes `dataList`,`groupList`, and `netDir` as input variables. The residual `...` parameter is to pass additional variables to `makePSN_NamedMatrix()`, notably `numCores`.
...
* splits data into train/blindtest using the default split of 80:20
...
**!!! Note:**Setting `numCores`>1 will not work in Rstudio. Use `knitr::purl("NestedCV_MultiData.Rmd")` to extract the R code into its own file, change `numCores` and run in an interactive R client (i.e. by calling "R" on command-line). **!!!**
...
Look into one of the split directories. `predictionResults.txt` contains blind test prediction labels for that split. There is one directory with cross-validation results per class
```{r}
