...
megaDir <- sprintf("%s/featSel_clinNets_%s",outRoot,dt)
...
for (rngNum in 1:40) {
    ...
    netList2 <- makePSN_NamedMatrix(dats_train$clinical, 
			...
			sparsify=TRUE,verbose=TRUE,numCores=numCores)
   
	# cat("Made clinical nets\n")
    cat(sprintf("Made %i clinical nets\n", length(netList2)))
	
	netList <- unlist(netList2)
...
# lock
#system(sprintf("chmod -R u-w %s",megaDir)
