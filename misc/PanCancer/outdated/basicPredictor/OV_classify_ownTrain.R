...
#' generates own train test split instead of using PanCancer ones.
...
inFiles <- list(
	...
	mirna=sprintf("%s/OV_miRNA_core.txt",inDir),
	rppa=sprintf("%s/OV_RPPA_core.txt",inDir),
	train=sprintf("%s/OV_train_sample_list.txt",inDir),
	test=sprintf("%s/OV_test_sample_list.txt",inDir)
)
