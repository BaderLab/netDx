...
netList <- dir(path=netDir,pattern=fileSfx)
	netID <- data.frame(ID = seq_len(length(netList)),
		...
		dir.create(paste(netDir,"INTERACTIONS",sep=.Platform$file.sep))
        for (p in netList) {
			dat <- read.delim(paste(netDir,p,sep=.Platform$file.sep),
				sep="\t",
			...
			write.table(dat2,
				file=paste(netDir,"INTERACTIONS",
					sprintf("1.%i.txt",netID$ID[which(netID$name == p)]),sep=.Platform$file.sep),
				sep="\t",col.names=FALSE,row.names=FALSE,quote=FALSE)
        }
    
    # write NETWORKS.txt
    write.table(netID, file = paste(netDir,"NETWORKS.txt",sep=.Platform$file.sep),
			sep = "\t", col.names = FALSE, 
        row.names = FALSE, quote = FALSE)
    
    # write NETWORK_GROUPS.txt
	con <- file(paste(netDir,"NETWORK_GROUPS.txt", sep=.Platform$file.sep), "w")
    write(paste(1, "dummy_group", "geneset_1", "dummy_group", 1, sep = "\t"),
	file = con)
    close(con)
    
    con <- file(paste(netDir,"NETWORK_METADATA.txt",sep=.Platform$file.sep), "w")
    tmp <- paste(netID$ID, "", "", "", "", "", "", "", 
			"", "", 0, "", "", 0, "", 
        "", "", "", "", sep = "\t")
