...
# Lasso+EucScale but not gene-level nets.
...
megaDir <- sprintf("%s/eucscale_sp2max%i_grouped_%s",outRoot,maxEdge,dt)
...
for (rngNum in 1:20) {
	...
	for (nm in names(dats_train)) { 
		...
		if (length(vars)>0) {
			...
			dats_train[[nm]] <- tmp
		} # else do nothing, use full dataset
		
		# one net for datatype
...
cat(sprintf("Making train nets for rng%i\n", rngNum))
