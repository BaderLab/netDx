...
megaDir <- sprintf("%s/ridgeExp_%s",outRoot,dt)

# ----------------------------------------------------------------
# helper functions


#' distance used in SNF
#' K (integer) num neighbours, should be between 10-30
#' sigma (numeric) scaling hyperparameter, should be between 0.3 and 0.8
sim.EucExp <- function(dat,K=20,sigma=0.3) {
sampNames <- colnames(dat)
dat <- t(dat)
# Euclidean distance, code from SNFtool package
dist2 <- function(X,C) {
  ndata = nrow(X)
  ncentres = nrow(C)
  ...
  return(res)
	...
	Diff <- dist2(as.matrix(dat),as.matrix(dat))
 	### code from SNFtool::affinityMatrix.
	### applies exponential scaling
  N = nrow(Diff)
  ...
  W = (densities + t(densities)) / 2
	colnames(W) <- sampNames
	rownames(W) <- sampNames
  return(W)
}
...
for (rngNum in 1:100) {
	...
	for (nm in setdiff(names(dats_train),"clinical")) {
		...
		if (length(vars) < 2) {# don't compute Pearson,just use all
...
browser()
	...
	netList <- makePSN_NamedMatrix(alldat,
		...
		simMetric="custom",customFunc=sim.EucExp,
		writeProfiles=FALSE,sparsify=TRUE)
	netList2 <- makePSN_NamedMatrix(alldat, 
		rownames(alldat),netSets_iter["clinical"],
		netDir,simMetric="custom",customFunc=sim.EucExp,writeProfiles=FALSE,
		verbose=FALSE,numCores=numCores,
		sparsify=TRUE,append=TRUE)
	netList <- c(netList,netList2)
	cat(sprintf("Total of %i nets\n", length(netList)))
