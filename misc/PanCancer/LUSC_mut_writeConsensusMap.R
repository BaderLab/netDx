...
datDir <- "/home/spai/BaderLab/PanCancer_LUSC/output/featSel_incMutRPPA_170219"

# limit to genes in dataset
mutFile <- "/home/spai/BaderLab/PanCancer_LUSC/input/from_firehose/LUSC_core_somatic_mutations.txt"
xprFile <- "/home/spai/BaderLab/PanCancer_LUSC/input/LUSC_mRNA_core.txt"
...
cat("----------------------\n")
	...
	# proteomics pathways
	cons <- cons[-idx]  # remove mutation variables
	idx <- grep("PROT_", unaccounted)
	unaccounted <- unaccounted[-idx]

	tmp <- sub("PROT_", "", cons)
	tmp <- sub("_cont","",tmp)
	pList <- pathwayList[which(names(pathwayList) %in% tmp)]
	names(pList) <- paste("PROT",names(pList),sep="_")
	cat(sprintf("\t > %i mutation based pathways\n",length(pList)))
	for (k in names(pList)) {
		cur <- pList[[k]]
		cur <- intersect(cur, mut_genes) # limit to interrogated genes
