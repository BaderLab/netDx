...
megaDir <- sprintf("%s/rbfclean_%1.2f_%s",outRoot,hp,dt)
...
rm(pheno)
rm(survStr,surv,tmp,nm,inDir,dt,k,inFiles,datFiles,pname,outRoot)

# -----------------------------------------------------------
# run predictor
source("PanCancer_rbf.R")

runPredictor(mega_combList=combList,rngVals=1:20,netSets=netSets,
	dats=dats,pheno_all=pheno_all,megaDir=megaDir,
	cutoffSet=9,sigmaVar=hp)
