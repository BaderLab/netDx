...
logFile <- sprintf("%s/%s_%s.log",outDir,curSet,dt)
...
for (aggFun in c("MAX","MEAN")) {
	for (simMode in c("normal","BinProp")) {
		...
		###	cat("****************************\n")
		###	cat("*** Excluding MUT nets   ***\n")
		###	cat("****************************\n")
		###	pTally <- pTally[-grep("^MUT_|stage|grade",pTally)]
		...
		if (simMode == "BinProp") {
			...
			isBinary[grep("MUT_|stage|grade",newNetIDs[,2])] <- 1
		...
		aggNetFile <- netDx::writeWeightedNets(ptFile$YES,
						...
						outFileName=sprintf("%s_%s_%s_PSN.txt", curSet,simMode,aggFun),
		...
		pyes <- wilcox.test(x$all[["SURVIVEYES"]],x$all[["SURVIVEYES-SURVIVENO"]],
										 alternative="less")$p.value
		pno <- wilcox.test(x$all[["SURVIVENO"]],x$all[["SURVIVEYES-SURVIVENO"]],
										 alternative="less")$p.value
		...
		write.table(pheno,file=sprintf("%s/%s_%s_%s_PSN_pheno.txt",
																	 outDir,curSet, simMode, aggFun),sep="\t",
					col=T,row=F,quote=F)
	}
}
},error=function(ex){ 
